...
type Props = {
  filter: UserTransferredFilter;
  setFilter: Dispatch<SetStateAction<UserTransferredFilter>>;
};

const NameCell = memo(({ row }: CellContext<UserTransferredRow, unknown>) => (
...
const RolesCell = memo(({ row }: CellContext<UserTransferredRow, unknown>) => (
...
const PositionsCell = memo(({ row }: CellContext<UserTransferredRow, unknown>) => (
  <div className="flex flex-wrap gap-1">
...
const DepartmentsCell = memo(({ row }: CellContext<UserTransferredRow, unknown>) => (
  <div className="flex flex-wrap gap-1">
...
const EmailCell = memo(({ row }: CellContext<UserTransferredRow, unknown>) => (
...
const PhoneCell = memo(({ row }: CellContext<UserTransferredRow, unknown>) => (
...
const useUserColumns = ({ filter, setFilter }: Props) => {
  const setFilterKey = useMemo(
    () =>
      debounce((keyName: keyof UserTransferredFilter, value: string) => {
  ...
  const columns = useMemo<ColumnDef<UserTransferredRow>[]>(() => [
    {
      id: 'ФИО',
      accessorKey: 'ФИО',
    ...
    {
      id: 'Дата перевода',
      accessorKey: 'Дата перевода',
      header: 'Дата перевода',
      size: 160,
      cell: ({ row }) => row.original.transferredAt ? dayjs(row.original.transferredAt).format('DD MMM YYYY') : '',
      sortingFn: (a, b) => (a.original.transferredAt || '').localeCompare(b.original.transferredAt || ''),
    },
    {
      id: 'Кто перевел',
      accessorKey: 'Кто перевел',
      header: 'Кто перевел',
      size: 220,
      cell: ({ row }) => row.original.transferredBy,
    },
    {
      id: 'Куда переведен',
      accessorKey: 'Куда переведен',
      header: 'Куда переведен',
      size: 220,
      cell: ({ row }) => row.original.transferredTo || '',
    },
    {
      id: 'Позиция',
      accessorKey: 'Позиция',
